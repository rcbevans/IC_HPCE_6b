...
namespace bitecoin{
	...
	bigint_t FastPoolHash(const Packet_ServerBeginRound *pParams, const uint32_t index, const uint64_t chainHash)
	{
		assert(NLIMBS==4*2);
		...
		// hash::fnv<64> hasher;
		// uint64_t localHash=hasher((const char*)&pParams->chainData[0], pParams->chainData.size());
		...
		wide_x_init(&x.limbs[0], index, pParams->roundId, pParams->roundSalt, chainHash);
	...
	bigint_t FastHashReference(
		...
		for(unsigned i=0;i<nIndices;i++){
			...
			bigint_t point=FastPoolHash(pParams, pIndices[i], chainHash);
